...
pub mod associated_polynomials {
    ...
    #[cfg(feature = "v2")]
    pub fn legendre_alp_deriv2(norm: enums::SfLegendreNorm, lmax: usize,
                               x: f64, result: &mut [f64], deriv: &mut [f64],
                               deriv2: &mut [f64]) -> enums::Value {
        unsafe {
            ffi::gsl_sf_legendre_deriv2_array(norm, lmax, x, result.as_mut_ptr(),
                                              deriv.as_mut_ptr(), deriv2.as_mut_ptr())
        }
    }
}
